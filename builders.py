...
def get_timezone_keyboard(current_start: int, is_setup: bool = False) -> InlineKeyboardMarkup:
    ...
    for tz in range(current_start, current_start + 6):
        ...
        if -12 <= tz <= 14:
            builder.button(text=f"{'+' if tz > 0 else ''}{tz}", callback_data=f"set_tz:{tz}:{setup_flag}")
    ...
    if current_start > -12:
        nav_buttons.append(builder.button(text="⬅️", callback_data=f"shift_tz:{current_start - 3}:{setup_flag}"))
